...
[documentation audit](../DOCUMENTATION_AUDIT.md) records what was fixed. D1 (the untagged `*_deviation` properties) was resolved by the
operational boundary in contract 0.2.0 ([contract audit](../CANONICAL_CONTRACT_AUDIT.md)).

| # | Intent says | Implementation does | Evidence | Status |
|---|---|---|---|---|
| D2 | `CanonicalState.snapshot(include_truth=False)` suggests a truth-free view is available | no route or caller uses `include_truth=False`; the operational view is built by `api/operational.py` instead | `simulator/state/__init__.py`, `api/operational.py` | documented; the flag is unused |
