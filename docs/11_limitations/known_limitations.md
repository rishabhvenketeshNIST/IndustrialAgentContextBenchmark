...
|---|---|---|---|---|---|
| L1 | **Ground truth reaches non-benchmark routes** (correlation ids, health, boundary values, true XMEAS history, exports, scenario files, lot deviations) | the separation was built for fault *control* and fault *events* only | a system reading the operational API can learn causes it should infer | **yes, critically**, if an agent is connected to the current API | an explicit observable view (allow-list) under its own route tree; strip correlation fields |
